fn git_init(dir: &std::path::Path) {
    ...
    std::process::Command::new("git").current_dir(dir).args(["config", "user.name", "test"]).output().unwrap();
    std::process::Command::new("git").current_dir(dir).args(["config", "user.email", "test@test.com"]).output().unwrap();
}

fn git_commit(dir: &std::path::Path, msg: &str) {
    std::process::Command::new("git").current_dir(dir).args(["add", "."]).output().unwrap();
    std::process::Command::new("git").current_dir(dir).args(["commit", "-q", "-m", msg]).output().unwrap();
}
...
fn capture_git_state() {
    ...
    git_init(&dir);
    std::process::Command::new("git").current_dir(&dir).args(["checkout", "-b", "test-branch"]).output().unwrap();
    std::fs::write(dir.join("file.rs"), "fn main() {}").unwrap();
    git_commit(&dir, "test commit");
